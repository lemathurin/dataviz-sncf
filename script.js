...
async function percentageHundredDays() {
  ...
  let resultSumFound;
  await fetch(
    "https://ressources.data.sncf.com/api/explore/v2.1/catalog/datasets/objets-trouves-gares/records?select=COUNT(*)%20as%20count&group_by=RANGE(date%2C%2020%20days)%20as%20date&order_by=date%20DESC&limit=5"
  )
    .then((response) => response.json())
    .then((data) => {
      //console.log(data)
  ...
  await fetch(
    "https://ressources.data.sncf.com/api/explore/v2.1/catalog/datasets/objets-trouves-restitution/records?select=COUNT(*)%20as%20count&group_by=RANGE(date%2C%2020%20days)%20as%20date&order_by=date%20DESC&limit=5"
  )
    .then((response) => response.json())
    .then((data) => {
      //console.log(data)
...
async function dataNantes() {
  await fetch(
    "https://ressources.data.sncf.com/api/explore/v2.1/catalog/datasets/objets-trouves-gares/records?select=date%2C%20COUNT(*)&where=%22Nantes%22&group_by=RANGE(date%2C%201%20day)%20as%20date&order_by=date%20DESC&limit=30"
  )
    .then((response) => response.json())
    .then((data) => {
      //console.log(data)
...
async function autocomplete(inp) {
  ...
  inp.addEventListener("input", function (e) {
    ...
    for (i = 0; i < arrayStation.length; i++) {
      ...
      if (
        arrayStation[i].substr(0, val.length).toUpperCase() == val.toUpperCase()
      ) {
        /*create a DIV element for each matching element:*/
        b = document.createElement("DIV");
        /*make the matching letters bold:*/
        b.innerHTML =
          "<strong>" + arrayStation[i].substr(0, val.length) + "</strong>";

        // Capitalize city names
        const cityName = arrayStation[i];
        const capitalizedCity =
          cityName.substr(0, 1).toUpperCase() +
          cityName.substr(1).toLowerCase();

        if (
          capitalizedCity.substr(0, val.length).toUpperCase() ===
          val.toUpperCase()
        ) {
          b.textContent = capitalizedCity; // Set the text content without manipulating HTML
          b.innerHTML +=
            "<input type='hidden' value='" + capitalizedCity + "'>";
...
async function getStation() {
  ...
  await fetch(
    `https://ressources.data.sncf.com/api/explore/v2.1/catalog/datasets/objets-trouves-gares/records?select=COUNT(*)%20as%20number%2C%20date&where=SEARCH(%22${station}%22)&group_by=RANGE(date%2C%201day)%20as%20date&order_by=date%20DESC&limit=1`
  )
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
  ...
  await fetch(
    `https://ressources.data.sncf.com/api/explore/v2.1/catalog/datasets/objets-trouves-restitution/records?select=COUNT(*)%20as%20number%2C%20date&where=SEARCH(%22${station}%22)&group_by=RANGE(date%2C%201%20day)%20as%20date&order_by=date%20DESC&limit=1`
  )
    .then((response) => response.json())
    .then((data) => {
